...
export class FastSimilarityMatcher extends AbstractMatchingAlgorithm {
    ...
    match(oldTree, newTree, matching, comparator) {
        ...
        for (const oldNode of oldNodes) {
            ...
            const oldLabel = oldNode.label === Dsl.ELEMENTS.CALL.label
                ? oldNode.label + oldNode.attributes.get(Dsl.CALL_PROPERTIES.ENDPOINT.label) : oldNode.label;
        ...
        for (const newNode of newNodes) {
            //Comparisons for call nodes are restricted to equal endpoints in fast match mode
            const newLabel =  newNode.label === Dsl.ELEMENTS.CALL.label
                ? newNode.label + newNode.attributes.get(Dsl.CALL_PROPERTIES.ENDPOINT.label) : newNode.label;
