...
export class StandardComparator extends AbstractComparator {
  ...
  comparePosition(node, other) {
    ...
    const nodePathSlice = node.path(radius).reverse().map(n => this.hashExtractor.getContentHash(n));
    const otherPathSlice = other.path(radius).reverse().map(n => this.hashExtractor.getContentHash(n));
