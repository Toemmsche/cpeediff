...
export class AggregateDiffResult {
    ...
    values() {
        return [this.algorithm, this.avgRuntime, this.maxRuntime, this.stdDevRuntime, this.avgCost, this.maxCost,
            this.stdDevCost, this.avgEditOperations, this.maxEditOperations, this.stdDevEditOperations];
    }

    static header() {
        return ["algorithm", "avg runtime", "max runtime", "runtime std dev", "avg rel cost", "max rel cost",
           "rel cost std dev", "avg rel edit operations", "max rel edit operations", "rel edit operations std dev"];
    }
    ...
    static of(diffResults) {
        ...
        let [avgEditOperations, maxEditOperations] = [changes.reduce((a, b) => a + b, 0) / changes.length, Math.max(...changes)];
